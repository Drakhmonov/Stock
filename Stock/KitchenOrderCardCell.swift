...
class KitchenOrderCardCell: UITableViewCell {
  static let reuseIdentifier = "KitchenOrderCardCell"
  ...
  func configure(with order: PlacedOrder, dateFormatter: DateFormatter) {
    ...
    let (text, color): (String, UIColor) = {
      if order.isPrepared  { return ("Prepared", .systemGreen) }
      if order.isPreparing { return ("Preparing", .systemOrange) }
    ...
    let items = order.items.map { "\($0.quantity)x \($0.name)" }
    ...
    // Times
    timeLabel.text = "Placed: " + dateFormatter.string(from: order.placedAt)
    if order.isPreparing, let at = order.preparingAt {
      timeLabel.text! += " • Prep: " + dateFormatter.string(from: at)
    }
    if order.isPrepared, let at = order.preparedAt {
      timeLabel.text! += " • Ready: " + dateFormatter.string(from: at)
    }
  }
}
